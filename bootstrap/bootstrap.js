...
function warnIfNeeded(browser, host) {
  ...
  if (blurtsDisabled || warnedHostSet.has(host) || !siteSet.has(host)) {
    return;
  }
  ...
  const ui = UIFactory[3](doc, host); // get from pref
  panel.addEventListener("popupshowing", () => {
    ...
    doc.getAnonymousElementByAttribute(n, "class", "popup-notification-icon").remove();
  });
...
let UIFactory = [
  function(doc, host) {
    return {
      get box() {
        ...
        let elt = doc.createElementNS(HTML_NS, "div");
        elt.setAttribute("style", "text-align: center; margin-bottom: 1rem;");
        let imgElt = doc.createElementNS(HTML_NS, "img");
        imgElt.setAttribute("src", imageDataURIs.iconLaptopRisk);
        imgElt.setAttribute("width", "64px");
        ...
        elt.setAttribute("style", "font-size: 1.3rem; text-align: center; white-space: pre;");
        box.appendChild(elt);
        elt = doc.createElementNS(XUL_NS, "description");
        elt.appendChild(doc.createTextNode("AllMusic was reported to Firefox Monitor, a service that collects information about data breaches and other ways hackers can steal your information.\n\nFind out if your account is at risk and learn how to protect yourself at the Firefox Monitor website."));
  ...
  function(doc, host) {
    ...
    retval.primaryAction = {
      ...
      accessKey: "f",
      callback: function() {
        doc.defaultView.openTrustedLinkIn("http://fx-breach-alerts.herokuapp.com/?breach=" + this._textbox.value, "tab");
      }.bind(retval),
    };
  ...
  function(doc, host) {
    ...
    retval.primaryAction = {
      ...
      accessKey: "f",
      callback: function() {
        doc.defaultView.openTrustedLinkIn("http://fx-breach-alerts.herokuapp.com/?breach=" + this._textbox.value + "&signup=" + this._checkbox.checked, "tab");
      }.bind(retval),
    };
    retval.secondaryActions = [
      {
