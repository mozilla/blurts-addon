...
let resourceURI;
function initSiteList(aResourceURI) {
  const path = aResourceURI.spec.substring("file://".length) + "breaches.json";
  OS.File.read(path).then(
    function onSuccess(array) {
      const text = (new TextDecoder()).decode(array);
      Cu.reportError(text);
      let sites = JSON.parse(text);
      for (let site of sites) {
        ...
        if (site.Domain == "linkedin.com") {
          Cu.reportError(domainMap.get("linkedin.com"));
        }
      }
      startObserving();
...
function warnIfNeeded(browser, host) {
  ...
  const ui = UIFactory[4](doc, host, domainMap.get(host)); // get from pref
  ...
  function(doc, host, site) {
    let retval = {
      ...
      get box() {
        ...
        elt2.setAttribute("src", resourceURI.spec + "PwnedLogos/" + domainMap.get(host).logoSrc);
...
function startup(aData, aReason) {
  ...
  initSiteList(aData.resourceURI);
}
